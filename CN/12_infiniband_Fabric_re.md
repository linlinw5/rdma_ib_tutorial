...
这里说的"路由"，**不是** IB 子网之间的路由，而是 IB 子网内部的最优路径计算问题，即某个 LID 到 另外一个 LID 的最优路径问题。

> **回顾一下第十章的内容**：
>
> 以太网 + TCP/IP 的世界里，跨网络通信靠的是 IP 地址和逐跳路由；InfiniBand 则完全不同，它是用一种叫 LID 的本地标识符，在链路层直接完成转发。
>
> 在 TCP/IP 里我们常说"二层交换，三层路由"。以太网的二层转发规则简单而粗暴，无非是"泛洪、学习、转发、老化"那一套 MAC 地址表机制；真正的路径选择要等到网络层（第三层）才登场，于是有了 RIP、EIGRP、OSPF、BGP 等各种路由算法，让数据包能逐跳挑出一条最优路径往下走。换句话说，TCP/IP 的路由解决的是子网之间的通信，在子网内部是简单粗暴的二层交换。
>
> InfiniBand 的世界则完全不同：链路层同时扛起了"交换"和"路由"两重职责：既要把数据快速送到正确的端口，又要完成路由寻址（由 SM 用路由算法预先算好每台 IB 交换机的转发表）。换句话说，路径计算在链路层就已经发生，而不是靠广播、泛洪那种"傻瓜式"的转发。至于跨子网的通信，那不过是"把网络做得更大"的问题罢了；而现实中 IB 网络很少需要多子网互通，单个子网的规模通常已经足够，能覆盖绝大多数场景。

SM 中负责路径决策逻辑的是 `路由引擎`，它的职责是：**在一个 IB 子网内部，计算每台交换机的转发表（LFT）该怎么填**。同一张物理拓扑，喂给不同的路由引擎，会算出不同的 LFT。OpenSM 内置了多种路由引擎（minhop、updn、ftree、torus-2QoS 等），通过启动参数 `-R <engine>` 选择。

这一章我们聚焦最基础、也最能说明问题的两种路由算法：min-hop 和 up-down，用一个精心设计的环形拓扑，看清它们的差别和各自的取舍。

## 12.2 实验拓扑：一个四交换机的环

我们用一个四台交换机首尾相连成环、每台各挂一个 HCA 的拓扑，ibsim 拓扑文件如下：
...
这套转发表里，每台交换机对每个目的 LID 都选了最短路，表面上看一切合理。
...
> **单独一条路径永远不会死锁，credit loop 必须是多股流量的占用方向叠加成环。**
...
从 min-hop 的转发表里，提取每台交换机转发到"对角交换机"的方向：
...
四个"占用→等待"首尾相接，绕环整整一圈，构成完整的循环等待。谁都在等下一个先释放，谁都动不了，结局就是死锁。
...
不过，这里必须说清楚：上面这个死锁是**潜在风险**，不是说这四股流量一出现就立刻死锁。真正触发需要临界条件：网络拥塞到这些链路的缓冲区都被填满、且这几股流量恰好同时争抢。平时流量不饱和时，包正常流过，看不出任何问题。

只要拓扑和路由**允许**这个循环依赖存在，它就是一颗定时炸弹。在足够高的负载、特定的流量组合下必然引爆。
...
updn 有个关键点，正好能用一个真实的报错来说明。如果直接用 `-R updn` 而不指定根，OpenSM 会报错并放弃：
...
注意倒数第二行：updn 失败后**回退（fallback）成了 minhop**。因此，如果不向 SM 指定根节点，你以为在跑 updn，实际跑的还是 minhop，转发表和 minhop 一模一样。

> updn 算法的第一原则：**没有根，就没有"上"和"下"的概念，方向无从定义，算法根本无法工作。**

### 指定根节点，让 updn 真正运行

我们在运行 openSM 的时候手动指定一个根。

```bash
# 选 SwA（GUID 0x200000）当根，写一个 root guid 文件：
...
从 updn 的路由表看，以 SwA 为根，环上形成了清晰的层级：**SwA 在顶层（根），SwB 和 SwD 是中间层（各距根一跳），SwC 在底层（距根两跳，是离根最远的对角点）。**
...
把这些改动对应前面分析的那个 credit loop，不难发现：updn 改掉的，**正是构成那个循环依赖的危险转弯**。一旦所有跨环流量都遵守"先上行到根、再下行到目的"，环上就再也凑不齐一整圈同方向的依赖，死锁被消除。

> SwC（对角点、底层）的转发表为什么不变？
>
> 因为 SwC 在最底层，它去任何节点本来就是纯粹的"上行"，不存在"下行后再上行"的风险，updn 无需改动它。

### 进阶

这里我还想再强调一点，在这个只有四个节点的小环上，当前 updn 的改动**并没有让任何路径的跳数变长**（都还是两跳）。

从结果看，目前改变的只是**路径方向的选择**，似乎没有牺牲最短性。那么试想一下，如果是 6 台交换机组成的环，updn 为了遵守"先上后下"的规则，是否会让某些流量走比最短路更长的路径呢？
