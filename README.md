...
用三节点 GPU 服务器做 AllReduce 抓包，把前面的 QP、带外 TCP 建连、RDMA Write、集合算法串成一条完整、可观测的数据流。先用全量抓包看清"建连三步"（rendezvous → NCCL bootstrap 交换 QPN/GID/rkey → RDMA），再用两组抓包对照看 Ring 与 Tree 数据层面的区别。

---

## 后记

**[后记：无损与有损——一场四十年的钟摆运动](CN/99_afterword_fabric_spiral.md)**
写完全书再回望，从部署现实与工程经济学的角度收尾。它不讲新的机制，而是把过去四十年"网络承担确定性"与"尽力而为加端侧智能"这架钟摆是怎么摆的讲清楚：训练与推理为何走向两种部署现实、无损 fabric 的"复杂半径"边界在哪里、PFC/ECN/DCQCN 该如何安放、UEC 又在偷师什么。最后回到一个问题：既然如此，为什么还要用一整本书去讲 InfiniBand。
...
- 相同方式共享（SA）：基于本指南内容创作的衍生作品，公开发布时必须继续采用 CC BY-SA 4.0 许可协议。
